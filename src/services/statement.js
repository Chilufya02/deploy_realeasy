...
function generatePaymentStatement(tenantName, payments = []) {
  const dateStr = new Date().toISOString().split('T')[0];
  const lines = [];
  lines.push('BT');
  lines.push('/F1 20 Tf 50 750 Td (Payment Statement) Tj');
  ...
  lines.push('120 0 Td (Amount) Tj');
  lines.push('80 0 Td (Status) Tj');
  lines.push('80 0 Td (Method) Tj');
  lines.push('100 0 Td (Reference) Tj');
  let y = 675;
  payments.forEach(p => {
    const date = new Date(p.date).toISOString().split('T')[0];
    lines.push(`1 0 0 1 50 ${y} Tm`);
    lines.push(`(${escapeText(date)}) Tj`);
    lines.push('120 0 Td (K ' + p.amount + ') Tj');
    const statusText = p.status === 'Received' ? 'Sent' : p.status;
    lines.push('80 0 Td (' + escapeText(statusText) + ') Tj');
    lines.push('80 0 Td (' + escapeText(p.method) + ') Tj');
    lines.push('100 0 Td (' + escapeText(p.reference || '') + ') Tj');
    y -= 15;
  });
  lines.push('ET');
...
module.exports = { generatePaymentStatement };
